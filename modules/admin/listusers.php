		...
		for ($j = 0; $j < mysql_num_rows($sql); $j++) {
			while($logs = mysql_fetch_array($sql,MYSQL_ASSOC)) {
				...
				<img style='border:0px;' src='${urlServer}/template/classic/img/arrow_grey.gif' title='bullet'></td>
    				<td>".htmlspecialchars(stripslashes($logs['nom']))."</td>
    				<td>".htmlspecialchars(stripslashes($logs['prenom']))."</td>
    				<td>".htmlspecialchars(stripslashes($logs['username']))."</td>
    				<td>".htmlspecialchars(stripslashes($logs['email']))."</td>
    				<td align='center'>";
				switch ($logs['statut'])
				{
				...
				<img src='../../template/classic/img/edit.gif' title='$langEdit' border='0'></a></td>
      				...
      				<td>
							<form id='myform".$logs['user_id']."' action='unreguser.php' method='post'>
    						<a href='javascript:;' onclick=\"document.getElementById('myform".$logs['user_id']."').submit();\">
								<img src='../../images/delete.gif' title='$langDelete' border='0'></img></a>
								<input type='hidden' name='u' value='".$logs['user_id']."'/>
