...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profname']))."&nbsp;".htmlspecialchars(stripslashes($req['profsurname']))."";
			$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profuname']))."</td>";
			if ($req['profemail'] != "") {
				$tool_content .= "<td>
				<a href=\"mailto:".htmlspecialchars(stripslashes($req['profemail']))."\">".
				htmlspecialchars(stripslashes($req['profemail']))."</a></td>";
			} else {
				htmlspecialchars(stripslashes($req['profemail']))."</td>";
			}
			$tool_content .= "<td>".htmlspecialchars(stripslashes($req['proftmima']))."</td>";
			$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profcomm']))."</td>";
	...
	// restore request
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			if ($req['profemail'] != "") {
				...
				htmlspecialchars(stripslashes($req['profemail']))."\">".
				htmlspecialchars(stripslashes($req['profemail']))."</a></td>";
			} else {
				$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profemail']))."</td>";
			}
			$tool_content .= "<td>".htmlspecialchars(stripslashes($req['proftmima']))."</td>";
			$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profcomm']))."</td>";
			$tool_content .= "<td align=\"center\">
...
// ---------------------------------
	...
	for ($j = 0; $j < mysql_num_rows($sql); $j++) {
		...
		$tool_content .= "\n    <td>".htmlspecialchars(stripslashes($req[1]))."&nbsp;".htmlspecialchars(stripslashes($req[2]))."</td>";
		for ($i = 3; $i < mysql_num_fields($sql) - 3; $i++) {
			if ($i == 4 and $req[$i] != "") {
				$tool_content .= "\n    <td><a href=\"mailto:".
				htmlspecialchars(stripslashes($req[$i]))."\">".
				htmlspecialchars(stripslashes($req[$i]))."</a></td>";
			} else {
				$tool_content .= "\n    <td>".
				htmlspecialchars(stripslashes($req[$i]))."</td>";
			}
		}
...
else
{
	...
	while ($req = mysql_fetch_array($sql)) {
		...
		<img style='border:0px;' src='${urlServer}/template/classic/img/arrow_grey.gif' title='bullet'></td>";
	     	$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profname']))."&nbsp;".htmlspecialchars(stripslashes($req['profsurname']))."</td>";
		$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profuname']))."</td>";
		if ($req['profemail'] != "") {
				$tool_content .= "\n<td><a href=\"mailto:".
				htmlspecialchars(stripslashes($req['profemail']))."\">".
			...
			$tool_content .= "\n<td>".htmlspecialchars(stripslashes($req['profemail']))."</td>";
		}
		$tool_content .= "<td>".htmlspecialchars(stripslashes(find_faculty_by_id($req['proftmima'])))."</td>";
		$tool_content .= "<td>".htmlspecialchars(stripslashes($req['profcomm']))."</td>";
		$tool_content .= "<td align='center'>
			<small>".nice_format(date("Y-m-d", strtotime($req['date_open'])))."</small></td>";
