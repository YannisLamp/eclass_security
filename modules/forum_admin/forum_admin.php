...
if(isset($forumgo)) {
	...
	if ($result and mysql_num_rows($result) > 0) {
		$tool_content .= "<form action=\"$_SERVER[PHP_SELF]?forumgoadd=yes&ctg=".htmlspecialchars($ctg)."&cat_id=".htmlspecialchars($cat_id)."\" method=post>
		...
		while(list($forum_id, $forum_name, $forum_desc, $forum_access,
			$forum_moderator, $forum_type) = mysql_fetch_row($result)) {
				...
				$tool_content .= "\n<td align='center'>
				<a href='$_SERVER[PHP_SELF]?forumgoedit=yes&forum_id=".htmlspecialchars($forum_id)."&ctg=".htmlspecialchars($ctg)."&cat_id=".htmlspecialchars($cat_id)."'>
				<img src='../../template/classic/img/edit.gif' title='$langModify' border='0'></img></a>&nbsp;
				<a href='$_SERVER[PHP_SELF]?forumgodel=yes&forum_id=".htmlspecialchars($forum_id)."&cat_id=".htmlspecialchars($cat_id)."&ctg=".htmlspecialchars($ctg)."&ok=0' onClick='return confirmation();'>
		...
		$tool_content .= "
		<form action=\"$_SERVER[PHP_SELF]?forumgoadd=yes&ctg=".htmlspecialchars($ctg)."&cat_id=".htmlspecialchars($cat_id)."\" method=post onsubmit=\"return checkrequired(this,'forum_name');\">
	...
	elseif(isset($forumgoedit)) {
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			if ($cat_id == $cat_id_1) {
					$tool_content .= "<option value='".htmlspecialchars($cat_id)."' selected>".htmlspecialchars($cat_title)."</option>"; 
				} else {
					$tool_content .= "<option value='".htmlspecialchars($cat_id)."'>".htmlspecialchars($cat_title)."</option>";
				}
			}
	...
	elseif (isset($forumcatsave)) {
		db_query("update catagories set cat_title='".mysql_real_escape_string($cat_title)."' where cat_id='".mysql_real_escape_string($cat_id)."'", $currentCourseID);
	...
	elseif(isset($forumgosave)) {
		...
		@db_query("UPDATE forums SET forum_name='".mysql_real_escape_string($forum_name)."', forum_desc='".mysql_real_escape_string($forum_desc)."',
            	forum_access='2', forum_moderator='1', cat_id='".mysql_real_escape_string($cat_id)."',
	...
	elseif(isset($forumcatadd)) {
		db_query("INSERT INTO catagories VALUES (NULL, '".mysql_real_escape_string($catagories)."', NULL)", $currentCourseID);
	...
	elseif(isset($forumgoadd)) {
		...
		@db_query("INSERT INTO forums (forum_id, forum_name, forum_desc, forum_access, forum_moderator, cat_id, forum_type)
        	VALUES (NULL, '".mysql_real_escape_string($forum_name)."', '".mysql_real_escape_string($forum_desc)."', '2', '1', '".mysql_real_escape_string($cat_id)."', '".mysql_real_escape_string($forum_type)."')", $currentCourseID);
		...
		<a href='$_SERVER[PHP_SELF]?forumgo=yes&cat_id=".htmlspecialchars($cat_id)."&ctg=".htmlspecialchars($ctg)."'>$langBack</a></p>";
	}
	...
	elseif(isset($forumcatdel)) {
		...
		db_query("DELETE FROM catagories where cat_id=".mysql_real_escape_string($cat_id)."", $currentCourseID);
	...
	elseif(isset($forumgodel)) {
		...
		$tool_content .= "\n<p class=\"success_small\">$langForumDelete<br />
			<a href=\"$_SERVER[PHP_SELF]?forumgo=yes&ctg=".htmlspecialchars($ctg)."&cat_id=".htmlspecialchars($cat_id)."\">$langBack</a></p>";
	} else {
		if(isset($forumcatnotify)) { // modify forum category notification
			$rows = mysql_num_rows(db_query("SELECT * FROM forum_notify 
				WHERE user_id = ".mysql_real_escape_string($uid)." AND cat_id = ".mysql_real_escape_string($cat_id)." AND course_id = ".mysql_real_escape_string($cours_id).""));
			if ($rows > 0) {
				db_query("UPDATE forum_notify SET notify_sent = '".mysql_real_escape_string($forumcatnotify)."' 
					WHERE user_id = ".mysql_real_escape_string($uid)." AND cat_id = ".mysql_real_escape_string($cat_id)." AND course_id = ".mysql_real_escape_string($cours_id)."");
			} else {
				db_query("INSERT INTO forum_notify SET user_id = ".mysql_real_escape_string($uid).",
				cat_id = ".mysql_real_escape_string($cat_id).", notify_sent = 1, course_id = ".mysql_real_escape_string($cours_id)."");
			}
		}
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			...
			$tool_content .= "\n<tr class=\"odd\">\n<td><div align='right'>$i.</div></td>
      			...
      			<td><div align='center'>".htmlspecialchars($numbers[total])."</div></td>
      			<td><a href='forum_admin.php?forumgo=yes&cat_id=".htmlspecialchars($cat_id)."&ctg=".htmlspecialchars($cat_title)."'>
			<img src='../../template/classic/img/forum_on.gif' border='0' title='$langForums'></img></a>&nbsp;
			<a href='$_SERVER[PHP_SELF]?forumcatedit=yes&cat_id=".htmlspecialchars($cat_id)."'>
			<img src='../../template/classic/img/edit.gif' border='0' title='$langModify'></img></a>&nbsp;
			<a href='$_SERVER[PHP_SELF]?forumcatdel=yes&cat_id=".htmlspecialchars($cat_id)."&ok=0' onClick='return confirmation();'>
			<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a>
			<a href='$_SERVER[PHP_SELF]?forumcatnotify=".htmlspecialchars($link_notify)."&cat_id=".htmlspecialchars($cat_id)."'>	
			<img src='../../template/classic/img/announcements".htmlspecialchars($icon).".gif' border='0' title='$langNotify'></img></a>
			</td></tr>";
			$i++;
		}
		$tool_content .= "</tbody></table></form><br/>
